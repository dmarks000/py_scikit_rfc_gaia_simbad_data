...
# astrtble = gdc.return_table_with_data(skyfile = 'ngc2420data.dat', skyfile2 = 'ngc2420data_sim.dat', coord = SkyCoord("07h38m24.5s +21 34 30", unit=(u.hourangle,u.deg), frame='icrs'))
...
input_table_base2 = "NGC_2420_probs.dat"
# ID and probability (blank lists)
with open(input_table_base2) as datfile:
     #dreader = csv.reader(csvfile)
     for line in datfile:
          if (line.find("Prob") != -1):
               #print("test")
          ...
          #print(line.split(','))
          probs = line.split(',')
          probs[1] = probs[1].replace('\n','')
          try:
               inde = CID.index(probs[0])
               mprob[inde] = float(probs[1])
          except:
               CID.append(probs[0])
               mprob.append(float(probs[1]))
     # CID.append(row[0])
     # mprob.append(float(row[1]))
     #print(mprob)
sizes = []
