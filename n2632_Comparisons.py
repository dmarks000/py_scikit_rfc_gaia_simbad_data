...
cluster = "NGC_2632"
clus2 = 'n2632'
input_table_base = 'NGC_2632_clean_gaia_data_no_rep.csv'
input_table_check = clus2 + '_stragglers_table.csv'
...
for i in range(len(cid2)):
    if cid2[i] in CID:
        ...
        finalmprob.append([mprob[cid_index],mprob2[i],mprobalt[cid_index]])
...
for i in range(len(finalcid)):
    flist.append([finalcid[i],finalmprob[i][0],finalmprob[i][1],finalmprob[i][2],finalparal[i]])
from operator import itemgetter, attrgetter
n_list = sorted(flist,key=itemgetter(1),reverse=True)
n_list.insert(0,["ID","Probability (Simbad)","Probability (Methodology)","Probability (Catagory 2)","Parallax (arcsec)"])
import tabulate
with open(clus2 + '_stragglers_Comparison.dat', "w") as f:
