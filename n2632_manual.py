...
center = [-4.537, -5.280]
goodradius = 0.2
definitelybadradius = 0.4
paralax = [5.5,5.0]

# read cone search results from a file
inputfilename = clus2 + '_cone.csv'
modelfilename = clus2 + '_model_file.data'
input_table_base = cluster + '_clean_gaia_data_no_rep_all.csv'

# import csv
ID=[];ra=[];dec=[];plx=[];eplx=[];pmra=[];epmra=[];pmdec=[];epmdec=[];g=[];bp=[];rp=[];bprp=[];rv=[];teff=[];ag=[];ebprp=[];vflag=[]
import Gaias_Data_Collect as gdc
astrtble = gdc.return_table_with_data(skyfile = 'm44data.dat', skyfile2 = 'm44data_sim.dat', coord = SkyCoord(ra=130.054, dec=19.621, unit=(u.degree, u.degree), frame='icrs'))
ID = astrtble["DESIGNATION"][astrtble['parallax'] > 0]
print(len(astrtble["DESIGNATION"]))
...
g = astrtble['phot_g_mean_flux'][astrtble['parallax'] > 0]
bp = astrtble['phot_bp_mean_flux'][astrtble['parallax'] > 0]
rp = astrtble['phot_rp_mean_flux'][astrtble['parallax'] > 0]
...
axis[1,0].hist([comp1,comp2],bins=100,range=(0.0,paralax[0]+0.3),stacked=True,color=['xkcd:light gray','xkcd:crimson'])
...
comp1=plx[rfmem <0.2]
comp2=plx[rfmem >= 0.2]
axis[1,0].hist([comp1,comp2],bins=100,range=(0.0,paralax[0]+0.4),stacked=True,color=['xkcd:light gray','xkcd:red'])
axis[1,0].set_title("Parallax")
axis[1,0].set_xlabel('Parallax (miliarcsec)')
axis[1,0].set_ylabel('N')
...
stragglers_data = []
stragglers_data.append(["ID",'In ' + cluster + '?',"BP-RP","G (mag)","PLX (arcsecs)","Alt Prob"])
