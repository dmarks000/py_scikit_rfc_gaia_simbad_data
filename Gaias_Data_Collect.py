...
def return_table_with_data(skyfile, skyfile2,coord,mode=0):
    if (mode<=1):
        try:
    ...
    if(mode>=1):
        try:
            ...
            custom_query.add_votable_fields('membership','pmra','pmdec','rvz_radvel','rv_value','rvz_qual','flux(B)','flux(V)','flux(K)','sp') #This adds the query data we do need. Some more specification needed to figure out what else we want.
